...
def sisgan(datatrain, subject, label, nseed):
    ...
    filename_subject = "pretrain_subject.cpt"
    state = torch.load(filename_subject)
...
input_subject = []
for num_s in range(input_data.shape[0]):
    train_subject.append(np.zeros(input_data.shape[1]) + num_s)

input_subject = np.asarray(input_subject)
num_subjects = input_subject.shape[0]
input_subject = input_subject.astype(np.int64)

train_data = np.concatenate(input_data)
train_label = np.concatenate(input_label)
train_subject = np.concatenate(input_subject)
...
gen_data, gen_label = sisgan(train_data, train_subject, train_label, seed_n)

data_filename = "SISGAN_CT.npy"
label_filename = "SISGAN_CT_labels.npy"
